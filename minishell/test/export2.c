...
/*   Updated: 2023/05/07 22:23:20 by hikaru           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	non_equal(char **cmd, t_list *env_head, int i)
{
	...
	while (cmd[i][j])
	{
		if (('a' <= cmd[i][j] && cmd[i][j] <= 'z') || ('A' <= cmd[i][j] && cmd[i][j] <= 'Z') || cmd[i][j] == '_' || ('0' <= cmd[i][j] && cmd[i][j] <= '9'))
			j++;
		else
		{
...
int	ft_export(char **cmd, t_list *env_head)
{
	...
	while (cmd[i])
	{
		if (ft_strcmp(cmd[i], "_") == 0)
			return (TRUE);
		if (('a' <= cmd[i][0] && cmd[i][0] <= 'z') || ('A' <= cmd[i][0] && cmd[i][0] <= 'Z') || cmd[i][0] == '_')
